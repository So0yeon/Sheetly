...
/* ──────────────────────────────────────────────────────────
   ...
   - 인쇄: 브라우저 인쇄 대화상자 (claude.ai 미리보기 샌드박스에서는
     print()가 차단될 수 있어 PDF 저장 버튼을 권장)
   ────────────────────────────────────────────────────────── */
...
/* ── 저장소: LocalStorage 우선, 막힌 환경(미리보기)에선 대체 저장소 ── */
const mem = {};
...
function friendlyError(e) {
  ...
  if (e.message === "NETWORK")
    return "네트워크 연결에 실패했어요. (claude.ai 미리보기에서는 외부 API가 차단됩니다 — 'API 설정'에서 Claude 미리보기 모드를 켜거나, 배포 후 사용해 주세요.)";
...
export default function App() {
  ...
  useEffect(() => {
    if (!ready) return;
    const t = setTimeout(() => saveKey("awm:settings", { opts, dark }), 600);
...
function PaginatedSheet({ headerEl, blockEls, teacher, editing, pagesRef }) {
  ...
  const compute = useCallback(() => {
    ...
    hs.forEach((h, i) => {
      ...
      cur.push(i);
      used += h;
      /* 블록 하나가 한 쪽보다 커도 그 쪽에 두고 넘김(overflow는 잘림 방지용 여유 설계) */
      if (used > cap) {
        out.push(cur);
        cur = [];
        used = 0;
...
.sheet-title{font-family:'Jua',sans-serif;font-weight:400;font-size:26px;margin:0 0 8px;line-height:1.3;color:#33383D}
